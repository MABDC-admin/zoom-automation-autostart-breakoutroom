...
export async function launchHostBot() {
  console.log(`\n========================================================`);
  console.log(`🤖 [OPENCLAW-HOST-BOT] Starting Room Opener with 3 Options...`);
  console.log(`========================================================`);
  ...
  // 4. In-Meeting Control Loop (Robust DOM Clicks & Option Management)
  console.log('[OPENCLAW] In-Meeting Control Loop Active...');
    ...
    try {
      ...
      for (const btn of buttons) {
        ...
        if (text && text.toLowerCase().includes('breakout room') && !roomsOpened) {
          ...
          if (optionsBtn) {
            console.log('🤖 [OPENCLAW] Clicked breakout settings cog...');
            await page.evaluate(el => el.click(), optionsBtn).catch(() => {});
            await new Promise(r => setTimeout(r, 1000));
          }

          // 2. Locate checkboxes uniquely via input[type="checkbox"] to prevent duplicate matches/toggling off
          ...
          // 3. Click "Open All Rooms" / "Start Rooms"
